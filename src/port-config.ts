...
import type {
  KnowledgeDashboardKit_ResolvedPort,
  KnowledgeDashboardKit_ResolvePortOptions,
} from "./types.js";
...
export function knowledgeDashboardKitParsePositivePort(options: {
  ...
  if (!/^\d+$/.test(normalized)) {
    throw new Error(
      `${options.errorPrefix} ${options.label} must be a positive integer port. Received '${options.rawValue}'.`,
    );
  }
  ...
  if (parsed < 1 || parsed > 65535) {
    throw new Error(
      `${options.errorPrefix} ${options.label} must be between 1 and 65535. Received '${options.rawValue}'.`,
    );
  }
  return parsed;
}
...
export function knowledgeDashboardKitStripEnvValue(rawValue: string): string {
...
export function knowledgeDashboardKitReadEnvFileValue(options: {
  ...
  for (const line of content.split(/\r?\n/)) {
    ...
    if (candidateKey === options.key) {
      resolved = knowledgeDashboardKitStripEnvValue(
...
function knowledgeDashboardKitActiveEnvFilePath(
  activeEnvFilePaths: readonly string[],
): string | null {
...
function knowledgeDashboardKitParseCliPort(
  options: KnowledgeDashboardKit_ResolvePortOptions,
): KnowledgeDashboardKit_ResolvedPort | null {
...
function knowledgeDashboardKitAssertRealmPortSafe(options: {
  readonly config: KnowledgeDashboardKit_ResolvePortOptions["config"];
  readonly port: number;
  readonly realmSlug: string;
}): void {
  if (
    options.realmSlug !== options.config.canonicalRealmSlug &&
    options.port === options.config.defaultPort
  ) {
    throw new Error(
      `${options.config.errorPrefix} Refusing to bind non-canonical realm '${options.realmSlug}' ` +
...
export function knowledgeDashboardKitResolvePort(
  options: KnowledgeDashboardKit_ResolvePortOptions,
): KnowledgeDashboardKit_ResolvedPort {
  ...
  const activeEnvFilePath = knowledgeDashboardKitActiveEnvFilePath(
    options.activeEnvFilePaths,
  );
  const activeEnvRealmSlug =
    knowledgeDashboardKitReadEnvFileValue({
      envFilePath: activeEnvFilePath,
      key: options.config.realmSlugEnvName,
    }) ?? options.config.canonicalRealmSlug;
  const realmSlug = (
    options.env[options.config.realmSlugEnvName] ?? activeEnvRealmSlug
  )
    .trim()
    .toLowerCase();
  ...
  const processEnvPort =
    options.env[options.config.canonicalPortEnvName] ??
  ...
  if (realmSlug !== options.config.canonicalRealmSlug) {
    throw new Error(
      `${options.config.errorPrefix} Missing ${options.config.canonicalPortEnvName} for ` +
